...
const [editTableUserEmail,setEditTableUserEmail] = useState('')
    ...
    const updateHandler = async (id) => {
        try {
            await updateUser({userId: id , username : editTableUserName , email : editTableUserEmail})
                       ...
                       {users.map((user) => (
                           <tr key={user._id}>
                               ...
                               <td className='border px-4 py-2'>
                                {editTableUserId===user._id ?(
                                    <div className="flex items-center" >
                                        <input type="text" value={editTableUserName} onChange={(e) => setEditTableUserName(e.target.value)} 
                               ...
                               <td className='px-4 py-2'>
                                {editTableUserId===user._id ? (
                                    <div className="flex items-center">
                                        <input type="text" value={editTableUserEmail} onChange={(e) => setEditTableUserEmail(e.target.value)} className='w-full p-2 border rounded-lg' />
                               ...
                               <td className='px-4 py-2'>
                                {!user.isAdmin ? (
                                    <div className='flex'>
                                        <button onClick={()=> deleteHandler(user._id)} className='bg-red-600 text-white font-bold py-2 px-4 rounded'>
                                            <FaTrash/>
                                        </button>
                                    </div>
                                ) : null}
